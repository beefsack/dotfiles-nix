...
- Token budget: 150k. Track your cumulative token usage; before it reaches 150k, stop at a clean point and report back with status "partially done" (or "blocked"), stating exactly what is complete, what remains, and how a fresh agent should pick it up. Never run past the budget hoping to finish.
- Run all commands in the foreground. Never use background execution, monitors, or wait on notifications - each build/test runs as a single foreground call with an explicit timeout, and you read its output directly.
