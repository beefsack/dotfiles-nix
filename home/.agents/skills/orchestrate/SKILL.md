...
3. Delegate to the subagent with a well-specified brief: the task,
   ...
   tests, linters and formatters itself, and to NOT commit or push -
   work must be reviewed first.
...
6. If further work is required, send it back to the same subagent via
   SendMessage with specific, actionable feedback. Repeat until the
   solution is correct, high quality, and meets acceptance criteria.
